...
function buildSystemPrompt(mem, hasImage, persona) {
  ...
  if (mem.learned_profile && Object.keys(mem.learned_profile).length > 0) {
    ...
    // Groq doesn't always follow the requested schema — sometimes returns
    // a string instead of an array for these fields. Normalize so .join() never throws.
    const toArr = v => Array.isArray(v) ? v : (v ? [v] : []);
    ['personality_traits','topics_of_interest','likes','dislikes','sensitive_topics','important_moments']
      .forEach(k => { lp[k] = toArr(lp[k]); });
